...
//存放任务信息
func Insert(taskname string,add string,category string,amount int,launchTime string)  error {
	var err error
	...
	_, err = Db.Exec(sqlStr,maxid+1,taskname,time.Now().Format("2006-01-02 15:04:05"),number)
	if err != nil {
		return err
	}
 fmt.Println("总数据插入成功")
