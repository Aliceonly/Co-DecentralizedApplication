...
发布任务者确认接受任务者完成任务
*/
func Gettaskhash(ins *contract.TaskDeployerContract,address common.Address,header *types.Header,taskname string,timestamp string){
	opts := bind.CallOpts{
		Pending:     true,
		From:        address,
		BlockNumber: header.Number,
	...
	hash,err:=ins.GetTxHash(ops,taskname,timestamp)
