...
实例化合约
 */
func Getsmartcontract() *contract.TaskDeployerContractStruct{
	ins, err := contract.NewTaskDeployerContractStruct(common.HexToAddress(contractadress), client)
	if err != nil {
		panic(err)
	}
	return ins
...
获取合约余额
 */
func GetcontractBanlance(ins *contract.TaskDeployerContractStruct, address common.Address, header *types.Header) (*big.Int)  {
	opts := bind.CallOpts{
		Pending:     true,
		From:        address,
...
获取任务列表函数
*/
func GetTasklist(ins *contract.TaskDeployerContractStruct, timestap *big.Int, address common.Address, header *types.Header) struct {
	Taskname    string
...
获取用户信息列表
*/
func Getuser(ins *contract.TaskDeployerContractStruct, timestap *big.Int, address common.Address, header *types.Header) struct {
	Name        string
	Phonenumber string
	Studentid   *big.Int
	Password    string
...
*/
func CreatNewEvent(
	ins *contract.TaskDeployerContractStruct,
...
func Userchangepsword(
   ...
   //修改名称
func Userchangename(ins *contract.TaskDeployerContractStruct,
	opts *bind.TransactOpts,
	name string,
	studentid *big.Int,
	password string) bool {
