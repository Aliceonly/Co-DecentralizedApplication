...
func transferEth(pk, fromAddress, toAddress string, weiValue *big.Int) (string, error) {
     a,b:=Getaccout2()
	privateKey, fromAddress, err := keyStoreToPrivateKey(a, &userA.Passwd)
   //加载私钥
	...
	//获得帐户的随机数
	nonce, err := client.ethConn.PendingNonceAt(context.Background(), publicfromAddress)
	if err != nil {
		log.Fatal("nonce ", err)
	}
