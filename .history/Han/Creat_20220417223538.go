...
func Creat(c *gin.Context){
	...
	taskplace2 :=c.PostForm("taskplace1")
	taskcontent :=c.PostForm("taskcontent")
	fmt.Println(taskname,tasktime,taskmoney,taskplace2,taskcontent)
	contract.CreatNewEvent(ins,Txopts,taskname+taskcontent+taskplace3,taskplace2,tasktime,n)
	times:=contract.Querytime(ins,adress,head)
	fmt.Print("----->",times)
	fmt.Print(contract.GetTasklist(ins,times,adress,head))
}
