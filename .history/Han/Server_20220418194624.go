...
func Start()error  {
	...
	dapp := r.Group("dapp")
	{
	   ...
	   dapp.POST("/querytasklist",Query)//调用合约直接查mapping方法
	   dapp.POST("/queryuselist",Queryuser)//调用合约直接查mapping方法
	   dapp.POST("/canceltask",Cancel)
	   dapp.POST("/queryselftask",Selftask)
	  }
