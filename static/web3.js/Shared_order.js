...
data = JSON.parse(sessionStorage.getItem('Query_Shared_order_detail'))
console.log(data);
if (data == undefined) {
    ...
    document.getElementById("Shared_order_detail").innerHTML = result;
}
...
function Read_more(e) {
    if (Account_status == "") {
        ...
        $.ajax({
            method: "post",
            url: "http://localhost:8080/dapp/Shared_order_Read_more",
            data: { timestamp: Timestamp },
            success: function (data) {
                console.log("success data", data);
                result = data.data
                sessionStorage.setItem('Shared_order_detail', JSON.stringify(data.data))
                console.log("result=====>", result);
                window.location.href="Shared_order_Read_more"
            },
            error: function (data) {
