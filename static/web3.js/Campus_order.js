...
function Read_more(e) {
    if (Account_status == "") {
        ...
        $.ajax({
            method: "post",
            url: "http://localhost:8080/dapp/Campus_order_Read_more",
            data: { timestamp: Timestamp },
            success: function (data) {
                console.log("success data", data);
                result = data.data
                sessionStorage.setItem('Campus_order_detail', JSON.stringify(data.data))
                console.log("result=====>", result);
                window.location.href="Campus_order_Read_more"
            },
            error: function (data) {
