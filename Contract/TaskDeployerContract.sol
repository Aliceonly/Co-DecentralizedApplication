...
contract TaskDeployer {
 ...
 //实现用户提前打钱到合约在通过合约打钱到用户的操作
   ...
   /*
    ...
    发布任务者确认接受任务者完成任务
     */
      function claimTrust(uint _timestamp,bytes memory _sigs,bytes32 _PHash,string memory _taskname) public payable {
        require(_tasklist[_timestamp].sponsor == msg.sender,"must be sponsor can confirm");
